 ...
 * distinct from a bad-credentials 401 from {@link RestAuthenticationEntryPoint}. The admin
 * API sits behind {@code ROLE_ADMIN} on top of that. CORS is scoped to the configured
 * origins, and both handlers return JSON so the browser never shows its native Basic-auth
 * popup.
 */
@Configuration
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception
    {
        http
                ...
                .authorizeHttpRequests(auth -> {
                    ...
                    // The admin app's API. Listed before the catch-all so it is gated on
                    // ROLE_ADMIN whichever branch below applies — including the relaxed dev
                    // one, where plain authentication would otherwise be enough.
