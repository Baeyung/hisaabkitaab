...
import io.github.baeyung.hisaabkitaab.dto.units.UnitNameRequest;
...
public class UnitController
{
    ...
    @PostMapping
    public ResponseEntity<UnitResponse> create(@Valid @RequestBody UnitNameRequest request,
            @CurrentStore(StoreRole.EDITOR) Store store)
    {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(UnitResponse.of(unitService.create(store, request.name())));
    }
