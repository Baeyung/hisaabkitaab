...
/**
 * End-to-end regression net: drives a real SALE through the store's {@code /event} and asserts
...
class EventFlowApiTest extends ApiTest
{
    ...
    {
        mvc.perform(post(api("any-store", "/event")).contentType(MediaType.APPLICATION_JSON)
    ...
    void saleRipplesToEveryReadSurface() throws Exception
    {
        ...
        mvc.perform(get(api(store, "/dashboard")).with(as(USER)))
                .andExpect(status().isOk());

        // Expense categories were seeded for the new store.
        mvc.perform(get(api(store, "/expense-categories")).with(as(USER)))
                .andExpect(status().isOk())
    ...
    {
        signup("3200000002");
        String store = createStore("3200000002", "Rana Cloth");
        mvc.perform(post(api(store, "/event")).with(as("3200000002"))
                        .contentType(MediaType.APPLICATION_JSON)
